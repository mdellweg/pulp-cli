...
The Pulp CLI is designed with a plugin structure. Plugins can either live in the main package or be shipped independently.
...
It abstracts the communication to the pulp server through its `api` property.
Further we encourage to abstract the communication with certain endpoints by subclassing the PulpEntityContext.
Those context should provide a common interface to the layer of `click` commands that define the user interaction.
...
The pulp CLI is designed to support a number of pulp server versions at the same time.
The entity contexts should be used to implement necessary server version depedent workarounds.
...
For certain regularly repeating patterns, like listing all entities of a kind, we provide generic commands that will use the underlying context objects.
